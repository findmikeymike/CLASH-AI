...
export const checkPaymentSystemHealth = async () => {
    try {
        ...
        const healthResults = data.results.map((result) => ({
        ...
        return {
            healthy: false,
            results: [],
            summary: "Health check failed with an unexpected error: " +
 ...
 */
async function checkPaymentIntentFunction() {
    try {
 ...
 */
async function checkUserMinutesFunction() {
    try {
        // Simple ping to check if the function is accessible
