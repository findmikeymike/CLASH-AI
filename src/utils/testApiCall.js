 ...
 */
export const simulateElevenLabsApiCall = async (duration = 500) => {
    ...
    try {
        ...
        await fetch(mockUrl);
        console.log(`Test API call simulation completed with duration ~${duration}ms`);
    }
    finally {
 ...
 */
export const listenForApiCallEvents = (callback) => {
    const handler = (event) => {
        callback(event);
    };
