...
const logOperation = async (operation: string, details: any, status: string, error: any = null) => {
...
export const trackCallUsage = async (
  userId: string,
  action: 'start' | 'end',
  callId?: string,
  characterId?: string
  ...
  try {
    ...
    try {
      ...
      if (action === 'start') {
        ...
        const existingCalls = JSON.parse(localStorage.getItem('call_sessions') || '[]');
        const callIndex = existingCalls.findIndex((call: any) => call.id === callId);
...
export const getRemainingMinutes = async (userId: string) => {
  try {
    // Log the operation start
...
export const addMinutes = async (userId: string, minutesToAdd: number) => {
  try {
    // Log the operation start
